    ...
    <section className="">
      {value.length < maxFiles && (
        <div
          {...getRootProps({
            className:
              "dropzone py-4 bg-red-500 text-yellow-300 font-secondary text-p4 px-4 rounded-lg border-4 border-dashed border-main-black",
          })}
        >
          <input {...getInputProps()} />
          <p>Drag & drop cat image here, or click to select</p>
        </div>
      )}
      {!!value?.length && (
