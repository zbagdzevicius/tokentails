...
const gameMusicMap: Record<GameType, string> = {
    SHELTER: "https://tokentails-nfts.fra1.cdn.digitaloceanspaces.com/music/music.mp3",
...
export const GameMusicPlayer = () => {
    ...
    useEffect(() => {
        const handleStorageChange = () => {
            ...
            const isMusicOn = savedMusicSetting ? JSON.parse(savedMusicSetting) : false;

            const audioElement = audioRef.current;
            if (!audioElement || !gameType) return;

            if (isMusicOn && isAllowedToPlay) {
                const musicUrl = gameMusicMap[gameType];
        ...
        <>
            {isAllowedToPlay && (
                <audio ref={audioRef} style={{ display: "none" }} loop>
                    <source src={gameType ? gameMusicMap[gameType] : ""} type="audio/mpeg" />
                </audio>
            )}
        </>
    );
