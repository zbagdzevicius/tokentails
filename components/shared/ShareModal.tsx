...
import { urlPrefix } from '@/constants/api';
...
export const ShareModal = ({ url, close }: IProps) => {
    ...
    const absoluteUrl = useMemo(() => process.env.NEXT_PUBLIC_DOMAIN + url.replace(urlPrefix, ''), [url]);
    ...
    return (
        <div className="fixed inset-0 w-full z-50 flex justify-center h-full md:hidden">
            ...
            ></div>
            <div className="z-50 w-full md:w-[480px] transition-from-bottom-animation max-w-full absolute bg-white absolute bottom-0 rounded-t-[22px] overflow-hidden shadow h-fit flex flex-col pb-8 pt-6">
                <FacebookShareButton url={absoluteUrl} hashtag={process.env.NEXT_PUBLIC_SITE_NAME}>
                    <SharedButtonWrapper>
                        <i className="bx bxl-facebook-circle text-h6"></i>
                        <div>Facebook</div>
                    </SharedButtonWrapper>
                </FacebookShareButton>
                <FacebookMessengerShareButton className="bg-accent" url={absoluteUrl} appId="722737458784658">
                    <SharedButtonWrapper>
